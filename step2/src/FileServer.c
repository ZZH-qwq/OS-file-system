...
int fs_process_cmd(User* user, CmdHeader* command, char* data) {
    int data_len, res;
    char* data_ptr = NULL;
#ifdef _DEBUG
    ...
    switch (command->type) {
        case FS_FORMAT:
            // Format the disk
            res = u_format_volume(user->vol);
            if (res >= 0) {
                free(user->cur_dir.dir_entries);
                free(user->cur_dir.orig_data);
        ...
        case FS_LS:
            // List directory
            res = u_list_dir(user, command->name_len == 2, &data_ptr, &data_len);
            if (res >= 0) {
...
int fs_print_work_dir(User* user) {
    char path[1060];
    sprintf(path, "\n\033[1;32mFileServer\033[0m:\033[1;34m%.1024s\033[0m$ ", user->path);
