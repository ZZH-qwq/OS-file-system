...
#endif
        // Check Premission
        bool user_prem = inode->i_uid == user->id && (inode->i_prem & USER_X) != 0;
        bool other_prem = inode->i_uid != user->id && (inode->i_prem & OTHER_X) != 0;
...
#ifdef _DEBUG
        printf("_check_cache:\tBack to home\n");
#endif
        free_dir(&user->cur_dir);
        _get_root_dir(user);
        res = _change_dir_relative(user, "home");
        if (res < 0) {
            print_err("_check_cache:\tFailed to change dir");
            return res;
        }
        _get_path(user);
        return FLUSH_DIR;
...
int _make_target_entry(User* user, char* targetname, u_int16_t mode) {
    DirType* dir = &user->cur_dir;
    bool user_prem = dir->inodeptr->i_uid == user->id && (dir->inodeptr->i_prem & USER_W) != 0;
    bool other_prem = dir->inodeptr->i_uid != user->id && (dir->inodeptr->i_prem & OTHER_W) != 0;
    if (user->id != 0 && !user_prem && !other_prem) {
        print_err("_make_target_entry:\tPermission denied");
        return PERMISSION_DENIED;
    }
...
int _remove_target_entry(User* user, int entry_idx, size_t len) {
    ...
    bool other_prem = dir->inodeptr->i_uid != user->id && (dir->inodeptr->i_prem & OTHER_W) != 0;
    if (user->id != 0 && !user_prem && !other_prem) {
        print_err("_remove_target_entry:\tPermission denied");
        return PERMISSION_DENIED;
    }
...
int _change_inode_prem(Volume* vol, Inode* inode, u_int16_t prem, u_int16_t id) {
    int res = read_inode(vol, inode);
    if (res < 0) {
        print_err("_change_inode_prem:\tFailed to read inode");
        return DISK_FAILURE;
    }
    if (id != 0 && inode->i_uid != id) {
        print_err("_change_inode_prem:\tPermission denied");
        return PERMISSION_DENIED;
    }
    inode->i_prem = prem;
    res = write_inode(vol, inode);
    if (res < 0) {
        print_err("_change_inode_prem:\tFailed to write inode");
        return DISK_FAILURE;
    }
    return 0;
}

int _change_inode_owner(Volume* vol, Inode* inode, u_int16_t new_id, u_int16_t id) {
    int res = read_inode(vol, inode);
    if (res < 0) {
        print_err("_change_inode_owner:\tFailed to read inode");
        return DISK_FAILURE;
    }
    if (id != 0 && inode->i_uid != id) {
        print_err("_change_inode_owner:\tPermission denied");
        return PERMISSION_DENIED;
    }
    inode->i_uid = new_id;
    res = write_inode(vol, inode);
    if (res < 0) {
        print_err("_change_inode_owner:\tFailed to write inode");
        return DISK_FAILURE;
    }
    return 0;
}