...
void write_to_socket(BasicDisk *disk, char *response) {
    int nbytes = write(disk->socket_fd, response, strlen(response));
    if (nbytes < 0) {
...
int read_sector(BasicDisk *disk, int cylinder, int sector) {
    ...
    // print the first SECOTR_SIZE bytes of the sector
    static char response[SECTOR_SIZE + 10];
    sprintf(response, "Yes %.*s\n", SECTOR_SIZE, sector_data);
...
int write_sector(BasicDisk *disk, int cylinder, int sector, int length, char *data) {
    ...
    if (length > str_len) {
        memset(data + str_len, 0, length - str_len);
    }
    update_track(disk, cylinder);
